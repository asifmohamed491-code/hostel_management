// AccountDetailsShared.tsx
//
// Shared building blocks for every role's Account Details page. Pulled
// out of the existing Student Account Details implementation
// (components/dashboard/content/student/StudentAccountDetails.tsx —
// left untouched) so Warden and Super Admin can reuse the exact same
// profile header, info-row mini-card, and Account Security card
// instead of a separate visual system per role.
"use client";
...
// Same body padding/gap used by every info card on the Student page.
export const CARD_BODY_CLASS =
  "px-[15px] py-4 sm:px-[19px] sm:py-5 flex flex-col gap-2 sm:gap-2.5";

export type IconComponent = ComponentType<SVGProps<SVGSVGElement>>;

export interface ProfileHeaderField {
  icon: IconComponent;
  label: string;
  value: string;
}

/** One labeled value row inside an info card — identical markup to the
 * Student page's InfoRow. */
...
/** Page title + "Dashboard / Account Details" breadcrumb, matching the
 * Student page exactly except for the dashboard href. */
export function AccountDetailsPageHeader({
  dashboardHref,
}: {
...
/** The glass profile summary card: avatar, name, role badge, and a row
 * of up to three quick-glance fields with dividers — same layout as
 * the Student page's profile card, decorative illustration included. */
...
/** The "Account Security" card with the Change Password action, routed
 * to the existing /forgot-password flow — identical to the Student
 * page's card. */
export function AccountSecurityCard({ icon }: { icon: ReactNode }) {
  return (
    <DashboardCard
